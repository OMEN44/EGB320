...
class Navigation(Node):
    ...
    def move_to_marker_apf(self, target_marker, target_distance, next_state):
        ...
        distance_to_marker = target_marker.distance
        middle_bearing = target_marker.bearing[1]  # middle bearing only

        # Build obstacle list (middle bearings only)
        all_obstacles = []
        for group in (self.obstacles, self.shelves):
            for obj in group:
                all_obstacles.append((obj.distance, obj.bearing[1]))  # distance, middle bearing
    ...
    def state_machine(self):
        ...
        if self.state == 'TURN_CALIBRATION':
            ...
            if abs(centre_bearing) < 0.01:  # ~1 degree tolerance
                self.publish_velocity(0.0, 0.0)
            ...
            bearing = self.aisle_markers[0].bearing[1]  # radians
            kp = 0.1
            ...
            # if abs(math.degrees(bearing)) < 0.05:
            #     self.publish_velocity(0.0, 0.0)
            #     self.state = 'DONE'
            # else:
            self.publish_velocity(0.0, rotation_velocity)
        ...
        elif self.state == 'MOVE_TO_AISLE_MARKER':
            phi = np.linspace(-np.pi, np.pi, 360) 
            self.target_marker = self.aisle_markers[self.aisle_index]
            distance_to_marker = self.target_marker.distance
            middle_bearing = self.target_marker.bearing[1]  # middle bearing only
            # while self.target_marker is not None:
            #     self.publish_velocity(forward_speed, 0.0)
            # # Build obstacle list (middle bearings only)
            # Gather all detected objects into one list
            goal = [distance_to_marker, middle_bearing]
            all_obstacles = []
            for group in (self.obstacles, self.shelves):
                if group:
                    all_obstacles.extend(group)   # merge lists
            U_rep = apf.repulsiveField(all_obstacles, phi)
            U_att = apf.attractiveField(goal, phi)
            best_bearing = apf.bestBearing(U_att, U_rep, phi)
